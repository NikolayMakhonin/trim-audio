...
function _normalizeAmplitudeWithWindow({ samplesData, channelsCount, channels, coef, maxMult, windowSamples, }) {
    ...
    function _normalize(i) {
        ...
        for (let j = 0; j < maxJ; j++) {
            ...
            const mult = Math.min(maxMult, max < audio_helpers.EPSILON ? maxMult : coef / max);
            const multPrev = Math.min(maxMult, maxPrev < audio_helpers.EPSILON ? maxMult : coef / maxPrev);
            ...
            for (let nChannel = 0; nChannel < channelsLength; nChannel++) {
                ...
                samplesData[index + channel] = audio_helpers.correctSample(value * _mult);
            }
        }
        ...
        for (let j = 0; j < maxJ; j++) {
            ...
            const mult = Math.min(maxMult, max < audio_helpers.EPSILON ? maxMult : coef / max);
            const multNext = Math.min(maxMult, maxNext < audio_helpers.EPSILON ? maxMult : coef / maxNext);
...
exports.normalizeAmplitudeWithWindow = normalizeAmplitudeWithWindow;
exports.normalizeAmplitudeWithWindowWorker = normalizeAmplitudeWithWindowWorker;
